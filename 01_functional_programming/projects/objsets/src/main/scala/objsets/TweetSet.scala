...
class Empty extends TweetSet: 
  ...
  override def mostRetweeted(max: Tweet) = max
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  override def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
     ...
     left.filterAcc(p, right.filterAcc(p, accl))
    }
  ...
  override def mostRetweeted: Tweet = mostRetweeted(elem)

  override def mostRetweeted(max: Tweet): Tweet = {
    val newMax = 
      if (elem.retweets > max.retweets) elem
      else max
    left.mostRetweeted(right.mostRetweeted(newMax))
  }
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet =  TweetReader.allTweets.filter(filterTweet(google))
  lazy val appleTweets: TweetSet =  TweetReader.allTweets.filter(filterTweet(apple))

  def filterTweet(keys: List[String]): Tweet => Boolean = t => keys.exists(w => t.text.contains(w))
